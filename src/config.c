...
bool dump_state( FHCTRL* fhctrl, const char* config_file ) {
	...
	for (i = 0; i < 128; i++) {
		...
		config_setting_set_int_elem    ( list, -1, fp->id );
		config_setting_set_int_elem    ( list, -1, fp->type );
		config_setting_set_string_elem ( list, -1, fp->name );
	}
